...
send_receive_kpb(std::vector<std::shared_ptr<fft::Gvec>>& gvec_kpb,
                 std::vector<mdarray<std::complex<double>, 3>>& wf_kpb, K_point_set& kset_wannier,
    ...
    for (int ik = 0; ik < kset_wannier.num_kpoints(); ik++) {
        for (int ib = 0; ib < nntot; ib++) {
            ...
            if (kset_wannier.ctx().comm_k().rank() == dest_rank) {
                ...
                wf_kpb.push_back(mdarray<std::complex<double>, 3>({gvec_kpb[index]->num_gvec(), num_bands, kset_wannier.ctx().num_spins()}));
            }

            tag = src_rank + kset_wannier.num_kpoints() * dest_rank;
            
            for ( int is = 0; is < kset_wannier.ctx().num_spins(); is++ ) {
                if (kset_wannier.ctx().comm_k().rank() == src_rank) {
                    req = kset_wannier.ctx().comm_k().isend(kset_wannier.get<double>(ikpb)->spinor_wave_functions().at(
                                                            memory_t::host, 0, wf::spin_index(is), wf::band_index(0)),
                                                            temp->num_gvec() * num_bands, dest_rank, tag);
                }
                if (kset_wannier.ctx().comm_k().rank() == dest_rank) {
                    kset_wannier.ctx().comm_k().recv(&wf_kpb[index](0, 0, is), gvec_kpb[index]->num_gvec() * num_bands, src_rank,
                                             tag);
...
calculate_Mmn(mdarray<std::complex<double>, 4>& M, K_point_set& kset_wannier, int const& num_bands,
              ...
              std::vector<mdarray<std::complex<double>, 3>> const& wf_kpb, std::vector<int> const& ikpb_index,
              int const& nntot, mdarray<int, 2> const& nnlist, mdarray<int, 3> const& nncell)
{
    ...
    for (auto it : kset_wannier.spl_num_kpoints()) {
        ...
        auto Swf_k     = std::make_unique<wf::Wave_functions<double>>(kset_wannier.get<double>(ik)->gkvec_sptr(),
                                                                      wf::num_mag_dims(kset_wannier.ctx().num_mag_dims()), 
                                                                      wf::num_bands(num_bands),
                                                                      kset_wannier.ctx().host_memory_t());

        auto ispin_step = 0; //warning! Need to be change for collinear case
        auto sr = (kset_wannier.ctx().num_mag_dims() == 3) ? wf::spin_range(0, 2) : wf::spin_range(ispin_step);
        
        apply_S_operator<double, std::complex<double>>(memory_t::host, sr, wf::band_range(0, num_bands), bp_gen,
        ...
        for (int ib = 0; ib < nntot; ib++) {
            ...
            /* get k+b pw_coeffs in a wavefunction */
            std::unique_ptr<wf::Wave_functions<double>> aux_psi_kpb = std::make_unique<wf::Wave_functions<double>>(
                    kset_wannier.get<double>(ik)->gkvec_sptr(), wf::num_mag_dims(kset_wannier.ctx().num_mag_dims()), wf::num_bands(num_bands),
                    kset_wannier.ctx().host_memory_t());
            ...
            for ( int is = 0; is < kset_wannier.ctx().num_spins(); ++is) {
                for (int ig = 0; ig < kset_wannier.get<double>(ik)->gkvec().num_gvec(); ig++) {
                    ...
                    for (int iband = 0; iband < num_bands; iband++) {
                        aux_psi_kpb->pw_coeffs(ig, wf::spin_index(is), wf::band_index(iband)) =
                                wf_kpb[index_ikpb](ig_, iband, is);
                    } //end iband
                } // end ig
            }//end is
            ...
            wf::inner(kset_wannier.ctx().spla_context(), mem, sr, *aux_psi_kpb, wf::band_range(0, num_bands),
...
K_point_set::generate_w90_coeffs() // sirius::K_point_set& k_set__)
{
    if ( ctx().num_mag_dims() == 1 ) {
        RTE_THROW("generate_w90_coeffs not implemented for nspin = 2, collinear");
    }
    ...
    for (int ik = 0; ik < int(kp.size(1)); ik++) {
...
//3-> non sense
    ...
    for (int iband = 0; iband < int(exclude_bands.size()); iband++) {
